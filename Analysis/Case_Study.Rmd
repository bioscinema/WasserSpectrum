...
HbA1c_seq <- seq(min(meta_df$HbA1c), max(meta_df$HbA1c), length.out = 100)
pred_df <- data.frame(HbA1c = HbA1c_seq)

perm_mat <- replicate(B, {
  dfp <- meta_df
  dfp$HbA1c <- sample(dfp$HbA1c)  # permute HbA1c (break association)
  fit <- wasserstein_spectrum(
    df = dfp, 
    feature_col = "Shannon", 
    outcome_col = "HbA1c", 
    confounder_cols = NULL,
...
tau_levels <- levels(factor(df_perm_long$tau))
idx_10 <- unique(round(seq(1, length(tau_levels), length.out = 10)))
breaks_10 <- tau_levels[idx_10]

p <- p +
  scale_x_discrete(
    breaks = breaks_10,
    labels = function(x) formatC(as.numeric(as.character(x)), format = "f", digits = 5)
  ) +
  theme(
...
B <- 1000
set.seed(1)
t_grid_show <- seq(0.1, 0.9, by = 0.1)
models <- wasserstein_spectrum(
  df = meta_df, 
  feature_col = "Shannon", 
  outcome_col = "HbA1c", 
...
set.seed(1)
t_grid_show <- seq(0.1, 0.9, by = 0.1)
models <-wasserstein_spectrum_multiclass(
  df = meta_df,
  feature_col = "Shannon",
  outcome_col = "Status",
  reference_level = "IHD",
...
#HbA1c_seq <- seq(min(meta_df$Status), max(meta_df$Status), length.out = 100)
#pred_df <- data.frame(Status = HbA1c_seq)

perm_mat <- replicate(B, {
  dfp <- meta_df
  dfp$Status <- sample(dfp$Status)  # permute HbA1c (break association)
...
perm_mat <- sapply(seq_along(taus), function(k){
  ...
  # residual permutation + refit full
  replicate(B, {
...
perm_mat <- replicate(B, {
  dfp <- meta_df
  dfp$BMI..kg.m.. <- sample(dfp$BMI..kg.m..)  # permute HbA1c (break association)
  fit <- wasserstein_spectrum(
    df = dfp, 
    feature_col = "Shannon", 
    outcome_col = "BMI..kg.m..", 
...
form_red  <- Shannon ~ Age + Gender + Depth
form_full <- Shannon ~ BMI..kg.m.. + Age + Gender + Depth

obs_coef <- sapply(taus, function(tau){
  fit <- rq(form_full, data = meta_df, tau = tau)
  unname(coef(fit)["BMI..kg.m.."])
})
