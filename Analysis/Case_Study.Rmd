...
```{r}
count_data <- as(otu_table(ps_filt), "matrix")
if (!taxa_are_rows(ps_filt)) {
  count_data <- t(count_data)  
}
sample_data(ps_filt)[["Depth"]] = colSums(count_data)
metadata <- data.frame(
  sample_id = sample_names(ps_filt),
  BMI       = as.numeric(sample_data(ps_filt)[["BMI..kg.m.."]]),
  Depth     = as.numeric(sample_data(ps_filt)[["Depth"]]),
  Age       = as.numeric(sample_data(ps_filt)[["Age"]]),
  Gender    = as.factor(sample_data(ps_filt)[["Gender"]])
)

# Remove rows with any NA in selected columns
valid_idx <- complete.cases(metadata[, c("BMI", "Depth", "Age", "Gender")])
metadata_clean <- metadata[valid_idx, ]

# Subset count data to match valid samples
count_data_clean <- count_data[, valid_idx]

dge <- DGEList(counts = count_data_clean)
dge <- calcNormFactors(dge)
design <- model.matrix(~ BMI + Depth + Age + Gender, data = metadata_clean)
v <- voom(dge, design, plot = TRUE) 
fit <- lmFit(v, design)
fit <- eBayes(fit)
# Test for BMI effect (coefficient named "BMI")
results <- topTable(fit, coef = "BMI", number = Inf)
...
#### G000436255(p = 1.637873e-18)

```{r}
TSS_data <- sweep(count_data_clean, 2, colSums(count_data_clean), FUN = "/")


df_otu <- data.frame(
  Count = count_data_clean["G000436255", ],
  BMI = metadata_clean$BMI,
  TSS = TSS_data["G000436255", ],
  Depth = metadata_clean$Depth,
  Age = metadata_clean$Age,
  Gender = metadata_clean$Gender
)
wasserstein_spectrum(
  df = df_otu, 
  diversity_col = "Count", 
...
#### G000372285 (p = 4.246911e-03)
```{r}
df_otu <- data.frame(
  Count = count_data_clean["G000372285", ],
  BMI = metadata_clean$BMI,
  TSS = TSS_data["G000372285", ],
  Depth = metadata_clean$Depth,
  Age = metadata_clean$Age,
  Gender = metadata_clean$Gender
)
...
#### G000169035 (p = 0.06155917)
```{r}
df_otu <- data.frame(
  Count = count_data_clean["G000169035", ],
  BMI = metadata_clean$BMI,
  TSS = TSS_data["G000169035", ],
  Depth = metadata_clean$Depth,
  Age = metadata_clean$Age,
  Gender = metadata_clean$Gender
)
...
# Try......
## summarize_wasserstein_curve
```{r}
summarize_wasserstein_curve <- function(result, metadata, outcome_col = "BMI") {
  # AUC
  auc <- pracma::trapz(result$quantiles, abs(result$beta1))

  # Linear slope
  slope <- coef(lm(result$beta1 ~ result$quantiles))[2]

  # Quadratic shape
  quad_fit <- lm(result$beta1 ~ poly(result$quantiles, degree = 2))
  linear_term <- coef(quad_fit)[2]
  curve_term <- coef(quad_fit)[3]

  trend_shape <- if (curve_term > 0) {
    if (linear_term > 0) "U-shaped (upward)"
    else if (linear_term < 0) "U-shaped (downward)"
    else "Symmetric U-shape"
  } else if (curve_term < 0) {
    if (linear_term > 0) "Inverted-U (rises then falls)"
    else if (linear_term < 0) "Inverted-U (falls then rises)"
    else "Symmetric inverted-U"
  } else {
    if (linear_term > 0) "Increasing linearly"
    else if (linear_term < 0) "Decreasing linearly"
    else "Flat"
  }

  # Wald test
  model_data <- data.frame(x = df_otu[[outcome_col]])
  p <- ncol(model.matrix(~ x, data = model_data))
  idx_vec <- result$idx_x + (0:(result$basis_df - 1)) * p

  beta_hat <- result$coef[idx_vec]
  vcov_hat <- result$vcov[idx_vec, idx_vec]

  wald_stat <- t(beta_hat) %*% MASS::ginv(vcov_hat) %*% beta_hat
  df_wald <- length(beta_hat)
  p_wald <- 1 - pchisq(wald_stat, df = df_wald)

  # Output summary
  list(
    AUC = round(auc, 4),
    LinearSlope = round(slope, 4),
    TrendShape = trend_shape,
    WaldStatistic = round(wald_stat, 4),
    DF = df_wald,
    p_value = signif(p_wald, 4)
  )
}
...
```{r}
df_otu <- data.frame(
  Count = count_data_clean["G000830295", ],
  BMI = metadata_clean$BMI,
  TSS = TSS_data["G000830295", ],
  Depth = metadata_clean$Depth,
  Age = metadata_clean$Age,
  Gender = metadata_clean$Gender
)
resultt = wasserstein_spectrum(
  df = df_otu, 
  diversity_col = "Count", 
  outcome_col = "BMI", 
  confounder_cols = c("Depth", "Age", "Gender"),
...
summarize_wasserstein_curve(resultt, df_otu)
```

## All taxs

```{r}
summarize_all_wasserstein <- function(otu_table, metadata,
                                      outcome_col = "BMI",
                                      confounder_cols = NULL,
                                      basis_df = 6,
                                      t_grid = seq(0.01, 0.99, length.out = 100),
  ...
  n_otus <- length(otu_names)

  for (i in seq_along(otu_names)) {
    otu <- otu_names[i]
    cat(sprintf("Running %d / %d: %s\n", i, n_otus, otu))  # 进度打印

    Count <- as.numeric(otu_table[otu, ])
    df_otu <- cbind(metadata, Count = Count)
    
    result <- wasserstein_spectrum(df = df_otu,
                                   diversity_col = "Count",
    ...
    # AUCs
    auc <- pracma::trapz(result$quantiles, result$beta1)
    abs_auc <- pracma::trapz(result$quantiles, abs(result$beta1))
    ...
    results_list[[otu]] <- data.frame(
      OTU = otu,
      AUC = round(auc, 4),
      abs_AUC = round(abs_auc, 4),
      WaldStatistic = round(wald_stat, 4),
      DF = df_wald,
      p_value = signif(p_wald, 4)
    )
...
```{r}
all_otu_result = summarize_all_wasserstein(count_data_clean, metadata_clean,"BMI", c("Depth", "Age", "Gender"))
head(all_otu_result)
write.csv(all_otu_result, file = "wasserstein_auc_summary.csv", row.names = FALSE)
```
...
# Make sure wasserstein_spectrum() is already loaded

compute_auc_per_otu <- function(count_data, metadata,
                                diversity_col_prefix = "OTU",
                                outcome_col = "BMI",
                                confounder_cols = NULL,
                                basis_df = 6,
                                t_grid = seq(0.01, 0.99, length.out = 100),
  ...
  otus <- rownames(count_data)
  if (length(otus) > max_otus) {
    otus <- otus[1:max_otus]
  }
  
  auc_results <- data.frame(OTU = character(), AUC = numeric(), stringsAsFactors = FALSE)
  
  for (otu in otus) {
    df <- data.frame(
      Count = as.numeric(count_data[otu, ]),
      metadata
    )
    ...
    if (!is.null(result)) {
      auc <- trapz(result$quantiles, result$beta1)
      auc_results <- rbind(auc_results, data.frame(OTU = otu, AUC = auc))
    } else {
      warning(sprintf("OTU %s failed to run wasserstein_spectrum", otu))
    }
  }
...
```{r}
auc_result = compute_auc_per_otu(count_data_clean, metadata_clean, confounder_cols = c("Depth", "Age", "Gender"), max_otus = 500)
auc_result$abs_AUC <- abs(auc_result$AUC)
auc_df_sorted <- auc_result[order(auc_result$abs_AUC), ]
(auc_df_sorted)
